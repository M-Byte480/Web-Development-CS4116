...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (array_key_exists('edit_firstname', $_POST)) {
        update_first_name_from_user_ID($user_ID, $_POST['edit_firstname']);
    }
    if (array_key_exists('edit_lastname', $_POST)) {
        update_last_name_from_user_ID($user_ID, $_POST['edit_lastname']);
    }
    ...
    if (array_key_exists('delete_picture', $_POST)) {
        delete_picture_by_user_ID($user_ID, $_POST['delete_picture']);
    }
    if (array_key_exists('add_picture', $_FILES)) {
        add_picture_by_user_ID($user_ID, base64_encode(file_get_contents($_FILES['add_picture']["tmp_name"])));
    }
}
